...
(function () {
  ...
  async function openFullscreenTab() {
    try {
      ...
      var win = window.open(url, 'passgp-pbi-fullscreen');
      if (!win) {
  ...
  function isFullscreenLink(el) {
    if (!el || el.tagName !== 'A') return false;
    if (el.id === FULLSCREEN_LINK_ID) return true;
    if (el.getAttribute('data-passgp-fullscreen') === '1') return true;
    var href = (el.getAttribute('href') || '').toLowerCase();
    return href.indexOf('kajabi-fullscreen.html') !== -1;
  }

  function installFullscreenClickCapture() {
    if (window.__PASSGP_FS_CAPTURE__) return;
    window.__PASSGP_FS_CAPTURE__ = true;
    document.addEventListener(
      'click',
      function (e) {
        var link = e.target.closest && e.target.closest('a');
        if (!link || !isFullscreenLink(link)) return;
        e.preventDefault();
  ...
  function bindFullscreenLink() {
    ...
    var links = document.querySelectorAll(
      '#' + FULLSCREEN_LINK_ID + ', [data-passgp-fullscreen="1"], a[href*="kajabi-fullscreen"]',
    );
